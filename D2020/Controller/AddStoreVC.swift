...
//        "description": desc , "arabic_description": arabicDesc!, "image": imgString ?? "",
...
//            formData.append(mail.data(using: .utf8) ?? Data(), withName: "email")
//
//            if self?.fileURL != nil{
//
...
//                        guard let profileLocalModel = try? jsonConverter.decode(OwnerStoreDetails.self, from: userProfileInJson!)else{
//                            KRProgressHUD.showError(withMessage: "لم يتم الحفظ")
//                            return
//                        }
...
//            }
    ...
    func citiesRequest(){
        ...
        Alamofire
            ...
            .response {[weak self] result in
                let jsonConverter = JSONDecoder()
                guard let apiResponseModel = try? jsonConverter.decode(CitiesForOwner.self, from: result.data!) else{return}
